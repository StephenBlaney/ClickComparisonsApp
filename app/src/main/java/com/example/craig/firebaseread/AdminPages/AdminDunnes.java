...
/*
 ...
 the user.
 */
public class AdminDunnes extends AppCompatActivity {
    /*
    ...
    */
    ...
    private String key="";
    private ArrayList<Dunnes> list;
    private String name="";
    private String tempname="";
    private ArrayAdapter<Dunnes> adapter;
    ...
    */
    protected void onCreate(Bundle savedInstanceState){
        ...
        setContentView(R.layout.activity_adddunnes);
        getIntent();
        setUpScreen();
        getDunnesList();
    }
   ...
   to the list view.
    */
    public void getDunnesList(){
        list = new ArrayList<>();
        adapter = new ArrayAdapter<>(getBaseContext(),android.R.layout.simple_list_item_1, list);
        listView.setAdapter(adapter);
        refDunnes.orderByKey().addChildEventListener(new ChildEventListener() {
            @Override
            public void onChildAdded(DataSnapshot dataSnapshot, String s) {
                Dunnes item = dataSnapshot.getValue(Dunnes.class);
    ...
    */
    public void addItem(){
        ...
        if(!addName.equals("")&& addPrice != 0.0 && !name.matches(addName)){
            Dunnes item = new Dunnes(addName,addPrice);
            Firebase newRef = ref.child("Dunnes").push();
    ...
    */
    public void editItem(){
         ...
         */
        Query query = refDunnes.orderByChild("name");
        ...
        */
        if(!nameEdit.equals("") && priceEdit != 0.0 && !name.matches(nameEdit)){
            Dunnes item = new Dunnes(nameEdit, priceEdit);
            refDunnes.child(key).setValue(item);
